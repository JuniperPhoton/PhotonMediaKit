...
class UIImageScrollView: UIScrollView {
    enum ScaleMode: Int {
        case aspectFill
        case aspectFit
        case widthFill
    ...
    func initialize() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(UIImageScrollView.changeOrientationNotification), name: UIDevice.orientationDidChangeNotification, object: nil)
    }
    ...
    @objc private func doubleTapGestureRecognizer(_ gestureRecognizer: UIGestureRecognizer) {
        ...
        if zoomScale >= minimumZoomScale * UIImageScrollView.kZoomInFactorFromMinWhenDoubleTap - 0.01 {
